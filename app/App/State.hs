...
setColours :: State -> Vector.Vector Colour -> IO ()
setColours (State ref) newColours = IORef.modifyIORef ref $ \state -> state {colours = newColours}

allColours :: State -> IO (Vector.Vector Colour)
allColours (State ref) = colours <$> IORef.readIORef ref

toggleSelected :: State -> Int -> IO ()
toggleSelected stateRef@(State ref) boxIdx = do
  wasSelected <- isSelected stateRef boxIdx
...
isSelected :: State -> Int -> IO Bool
isSelected (State ref) boxIdx = (Set.member boxIdx . selected) <$> IORef.readIORef ref
